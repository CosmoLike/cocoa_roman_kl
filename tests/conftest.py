...
flags:
    ...
    python -m pytest ./projects/roman_kl/tests --high=1 --mask=ones
...
# the masks the comparison sweep can run under; "frozen" keeps each
# example's frozen contract mask, and "ones" keeps every data point
# (no scale cuts). A non-frozen choice swaps each example's TATT
# dataset for its frozen "<stem>_<mask>.dataset" variant (the
# machinery lives in cocoa_testing.CocoaTestHarness).
...
def pytest_addoption(parser):
    ...
    mask, and --mask=ones keeps every data point (no scale cuts).
    ...
    try:
        parser.addoption(
            ...
            help="the scale-cut mask of the CFASTPT-vs-FASTPT "
                 "comparison: frozen (each example's contract mask, "
                 "the default) or ones (every data point kept)")
    except ValueError:
        pass
